...
export default function PainBodyMap() {
  ...
  const quickMuscles = view === "anterior" ? ANTERIOR_MUSCLES : POSTERIOR_MUSCLES;
  ...
  const painStats = useMemo(() => {
    if (painPoints.length === 0) {
      return { max: 0, average: 0, label: "ยังไม่มีข้อมูล" };
    }
    const max = Math.max(...painPoints.map((point) => point.level));
    const average =
      painPoints.reduce((total, point) => total + point.level, 0) / painPoints.length;
  ...
  const selectOrCreateRegion = useCallback((region: PainRegion) => {
    setPainPoints((prev) => {
      if (prev.some((point) => point.region === region)) {
        return prev;
      }
      return [
        ...prev,
  ...
  const removePart = useCallback((region: PainRegion) => {
    setPainPoints((prev) => prev.filter((point) => point.region !== region));
    setSelectedRegion((prev) => (prev === region ? null : prev));
  }, []);

  const updateLevel = useCallback((region: PainRegion, level: number) => {
    setPainPoints((prev) =>
      prev.map((point) => (point.region === region ? { ...point, level } : point))
    );
  }, []);
          ...
          {selectedPainPoint ? (
            <div className="rounded-xl border border-primary/30 bg-primary/5 p-4">
              <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
                ...
                <button
                  type="button"
                  onClick={() => removePart(selectedPainPoint.region)}
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                      <span
                        className={`rounded-full border px-2 py-1 text-xs font-semibold ${severity.tone}`}
                      >
                        {point.level}/10
                      </span>
                      <button
                        type="button"
                        onClick={() => removePart(point.region)}
